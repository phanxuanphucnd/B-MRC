...
def main(args, tokenizer):
    ...
    if args.mode == 'test':
        ...
        for epoch in tqdm(range(start_epoch, args.epoch_num+1)):
            ...
            f1 = test(model, tokenizer, batch_generator_dev, dev_standard, args.inference_beta)
            ...
            f1 = test(model, tokenizer, batch_generator_test, test_standard, args.inference_beta, mode='test')

    else:
        logger.error('Error mode!')
        exit(1)
        
def test(model, t, batch_generator, standard, beta, mode='dev'):
    model.eval()
